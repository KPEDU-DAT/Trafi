    ...
    <h1>Ajoneuvotiedot</h1>
        <p>
            <form action="hcbody.php" method="POST">
                <input type="text" placeholder="Anna VIN-koodi:"
name="valmistenumero2">
                <p><button class="secondary button" type="submit"
...
mysqli_connect_error());
            ...
            if ($_POST['valmistenumero2']) {
                $VIN = mysqli_real_escape_string($yhteys,
$_POST['valmistenumero2']);
                $tulos = mysqli_query($yhteys, "SELECT *
                                                FROM trafi_ajoneuvot,
trafi_vari
                                                WHERE
trafi_ajoneuvot.valmistenumero2 = '" . $VIN . "'
                                                AND trafi_ajoneuvot.vari =
trafi_vari.koodintunnus
                                                ORDER BY kayttoonottopvm
DESC");
        echo
"<tr><th>Merkki</th><th>Malli</th><th>Ensirekisteröintipvm</th><th>Väri</th><th>Ajoneuvoluokka</th><th>Ovienlkm</th><th>Istumapaikkojenmäärä</th><th>Omamassa</th><th>Iskutilavuus</th>";                                        
